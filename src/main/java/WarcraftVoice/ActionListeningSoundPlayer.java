...
public class ActionListeningSoundPlayer implements
    ...
    public ActionListeningSoundPlayer(Playlist sounds, Listener listener) {
        this.sounds = sounds;
    ...
    public void changePlaylist(Race race) {
        if (race == sounds.getCurrentRace()) return;
        sounds.background.stop();
        sounds = Playlist.create(race);
        sounds.background.playInBackground();
    }
...
//            sounds.coin.play();
    ...
    @Override public void onUnitTestSucceeded() {
        sounds.onUnitTestSucc.play();
    }

    @Override public void onUnitTestFailed() {
        sounds.onUnitTestFail.play();
    }

    @Override public void onVcsCommit() {
        sounds.onVcsCommit.play();
    }

    @Override public void onVcsUpdate() {
        sounds.onVcsUpdate.play();
    }

    @Override public void onVcsPush() {
        sounds.onVcsPush.play();
    }

    @Override public void onVcsPushFailed() {
        sounds.onVcsPushFail.play();
    }



    private static Map<String, Sound> refactoringSounds(Playlist sounds) {
...
//        result.put("refactoring.introduceParameter", sounds.coin);
    ...
    private static Map<String, Sound> editorSounds(Playlist sounds) {
        Map<String, Sound> result = new HashMap<String, Sound>();

//        result.put("EditorUp", sounds.kick);
//        result.put("EditorDown", sounds.kick);
//        result.put("EditorUpWithSelection", sounds.kick);
//        result.put("EditorDownWithSelection", soundsWarcraft 3 Soundtrack - .kick);
